 ...
 *
 * <p>Отклонение от брифа: явный {@code SXSSFWorkbook.dispose()} опущен — в POI 5.3 он
 * deprecated (временные файлы удаляются в {@code close()}, которым управляет
 * try-with-resources), а сборка с {@code -Xlint:all -Werror} не терпит предупреждений.
 */
